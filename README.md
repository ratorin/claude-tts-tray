...
python temp/gen_icon.py                 # アイコン再生成（.ico+.png、初回のみ）
...
```bash
./build-linux.sh        # → dist/claude-tts-tray-linux-amd64 (ELF, static)
```

**Linuxへインストール（デスクトップ環境が必要）:**
```bash
./install-linux.sh      # ~/.local/bin に配置 + XDG autostart 登録
./uninstall-linux.sh    # 削除
```
